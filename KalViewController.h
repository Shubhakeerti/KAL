...
-(void)didSelectCalendarDate:(NSDate*)date;
@end
...

{
    ...
    // I cache the selected date because when we respond to a memory warning, we cannot rely on the view hierarchy still being alive, and thus we cannot always derive the selected date from KalView's selectedDate property.
...
@property (nonatomic, assign) id calendarDelegate;
@property (nonatomic) NSInteger followingIndex;
